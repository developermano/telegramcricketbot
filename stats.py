...
def playerstat(msg,format):
    try:
        ...
        image= "https://cricbuzz.com"+images[0]['src']    #3
        ...
        if format=="T20":
            return [name,country,image,role,tmatches,truns,ths,tavg,tsr,t100,t50,tballs,tbruns,twickets,tbbi,tecon,t5w]
        if format=="ODI":
            return [name,country,image,role,odimatches,odiruns,odihs,odiavg,odisr,odi100,odi50,odiballs,odibruns,odiwickets,odibbi,odiecon,odi5w]
        if format=="TEST":
            return [name,country,image,role,testmatches,testruns,tesths,testavg,testsr,test100,test50,testballs,testbruns,testwickets,testbbm,testecon,test5w]
